...
function Rating({ artistScore, venueScore, artist, venue }: RatingProps) {
  ...
  function showScore(score: number) {
    if (score != null && score > 0.5) {
      ...
      return Array.from({ length: 5 }).map((_, i) => <Star key={i} size={35} fill={"none"} />);
    }
  }
  ...
  return (
    <div className={styles.rateBox}>
      <Link href={`/ratings/venues/${venue.venueID}`} className={styles.box}>
        <div className={styles.text}>{venue.venueName}</div>
        <div className={styles.stars}>{showScore(venueScore)}</div>
      </Link>
      <Link href={`/ratings/artists/${artist.artistID}`} className={styles.box}>
        <div className={styles.text}>{artist.name}</div>
        <div className={styles.stars}>{showScore(artistScore)}</div>
      </Link>
    </div>
  );
}
