...
export default function Settings() {
  ...
  async function onSaveDescription(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(
      environment.backendURL + "/profile/settings/personal/description",
      {
        method: "PATCH",
        body: formData,
        mode: "cors",
  ...
  function PersonalSettings() {
    return (
      <>
        <div>
          <div className={styles.personalSettingTitle}>Personal information</div>
          <div className={styles.personalSetting}>
            <form onSubmit={onSaveMail}>
              <div className={styles.settingName}>Your e-mail:</div>
              <div className={styles.settingValue}>{user && user.mail}</div>
          ...
          <div className={styles.personalSetting}>
            <div className={styles.settingName}>Change Password:</div>
          ...
          <div className={styles.personalSetting}>
            <form onSubmit={onSaveDescription}>
              <div className={styles.settingName}>Your biography:</div>
              <div className={styles.settingValue}>{user && user.description}</div>
              <div className={styles.changeText}>
                <textarea
                  className={styles.changeBiographyContainer}
                  id="description"
                  name="description"
                  rows={3}
